<div>
    ...
    Livewire.on('trigger-save', () => {
        ...
        <div class="relative z-10" x-data="{ tab: @entangle('abaAtiva') }">{{-- WIRE:POLL DEIXAR EM 60, DEPENDENDO EU REDUZO PRA 20, OU 15 --}}
            <a class="ml-[4px] flex-1 px-5 w-20 h-fit py-1 border border-red-800/20 rounded hover:bg-red-900/10 transition text-base text-red-900 font-bold"
            ...
            <div class="h-fit flex justify-between items-center mb-4 p-1 border-b-4 border-red-900/40">
                ...
                <div class="bg-transparent">
                    <input type="text" wire:model.live="dados.nome"
                ...
                <div class="flex gap-4">
                    <div class="border-l-2 pl-2 border-red-900/40">
                        ...
                        <input type="number" wire:model.live="dados.nivel"
                            class="w-8 bg-transparent font-bold border-none focus:ring-0 text-red-950"
                    ...
                    <div class="border-l-2 pl-2 border-red-900/40">
                        ...
                        <input type="text" wire:model.live="dados.raca"
                            class="w-24 bg-transparent font-bold border-none focus:ring-0 text-red-950">
                    </div>
                    <div class="border-l-2 pl-2 border-red-900/40">
                        <label class="text font-bold uppercase text-[12px] block text-red-800">Classe</label>
                        <input type="text" wire:model.live="dados.classe"
                            class="w-48 bg-transparent font-bold border-none focus:ring-0 text-red-950">
                    </div>
                    <div class="border-l-2 pl-2 border-red-900/40">
                        <label class="text font-bold uppercase text-[12px] block text-red-800">Divindade</label>
                        <input type="text" wire:model.live="dados.divindade"
                            class="w-24 bg-transparent font-bold border-none focus:ring-0 text-red-950">
                    </div>
                </div>
                ...
                <div class="col-span-1 md:col-span-2 space-y-2">
                    @foreach (['forca', 'destreza', 'constituicao', 'inteligencia', 'sabedoria', 'carisma'] as $attr)
                        <div class="border-2 border-red-900/30 bg-white/10 p-2 rounded text-center">
                            ...
                            <input type="number" wire:model.live="dados.{{ $attr }}"
                    ...
                    <div class="grid grid-cols-4 gap-2 mb-4 text-center">
                        <div class="bg-red-900/5 p-2 border border-red-900/40 font-bold">
                            ...
                            <div class="flex justify-center">
                                <input type="number" wire:model.live="dados.hp_atual"
                                    class="w-8 bg-transparent text-center text-lg font-bold border-none focus:ring-0 text-red-950">
                                <strong class="text-red-800">/</strong>
                                <input type="number" wire:model.live="dados.hp_maximo"
                        ...
                        <div class="bg-purple-900/5 p-2 border border-purple-900/40 font-bold">
                            ...
                            <div class="flex justify-center">
                                <input type="number" wire:model.live="dados.estresse_atual"
                                    class="w-8 bg-transparent text-center text-lg font-bold border-none focus:ring-0 text-red-950">
                                <strong class="text-purple-800">/</strong>
                                <input type="number" wire:model.live="dados.estresse_maximo"
                                    class="w-8 bg-transparent text-lg text-center border-none focus:ring-0 text-red-950">
                            </div>
                        ...
                        <div class="bg-blue-900/5 p-2 border border-blue-900/40 font-bold">
                            ...
                            <div class="flex justify-center">
                                <input type="number" wire:model.live="dados.mp_atual"
                                    class="w-8 bg-transparent text-center text-lg font-bold border-none focus:ring-0 text-red-950">
                                <strong class="text-blue-800">/</strong>
                                <input type="number" wire:model.live="dados.mp_maximo"
                                    class="w-8 bg-transparent text-lg text-center border-none focus:ring-0 text-red-950">
                            </div>
                        </div>
                        <div class="bg-gray-900/5 p-2 border border-gray-900/40 font-bold">
                            ...
                            <div class="flex justify-center">
                                <input type="number" wire:model.live="dados.defesa"
                                    class="w-8 bg-transparent text-lg text-center font-bold border-none focus:ring-0 text-red-950">
                            </div>
                            <p class="ml-auto mr-auto text-center text-[13px] w-fit text-gray-800/80">
                                10+DES+Armadura<br>+Escudo+Outros </p>
                        </div>
                    </div>
                    ...
                    <div class="p-4 bg-white/5 border-2 border-red-900/30 rounded min-h-[400px]">
                            ...
                            @foreach ($pericias as $idx => $pericia)
                                <div class="flex items-center border-b border-red-900/10 py-1 gap-2">
                                    ...
                                    <div class="flex items-center gap-2 flex-1"
                                        wire:key="pericia-{{ $pericia['id'] }}">
                                        <input type="checkbox" wire:model.live="pericias.{{ $idx }}.treinado"
                                            class="accent-red-800">
                                        <span
                                            class="text-xs uppercase font-semibold flex-1 text-red-950">{{ $pericia['nome'] }}</span>
                                        <div
                        ...
                        <div x-show="tab === 'ataques'" class="space-y-2">
                            ...
                            @foreach ($ataques as $att)
                                <details wire:ignore wire:key="ataque-{{ $att['id'] }}"
                                    ...
                                    <summary
                                        class="uppercase p-2 font-bold cursor-pointer text-red-950 bg-red-950/10 flex items-center">
                                        {{ $att['nome'] }}

                                        <a href="{{ route('ataque.edit', ['id' => $att['id'], 'tab' => 'ataques']) }}"
                                            class="inline-flex items-center justify-center w-[30px] h-[30px] ml-auto hover:bg-red-900/20 rounded transition">
                                            <svg class="w-5 h-5 fill-red-950" xmlns="http://www.w3.org/2000/svg"
                                                viewBox="0 0 640 640">
                                                <path
                        ...
                        <div x-show="tab === 'magias'" class="space-y-2">
                            ...
                            @foreach ($magias as $magia)
                                <details wire:ignore
                                    class="mb-2 border-l-4 border-red-800 bg-white/10 overflow-hidden">
                                    <summary
                                        class="uppercase p-2 font-bold cursor-pointer text-red-950 bg-red-950/10 flex items-center">
                                        {{ $magia['nome'] }} - <span
                                            class="text-xs ml-2 opacity-70">{{ $magia['circulo'] }}º
                                            Círculo</span>

                                        <a class="ml-auto p-1 hover:bg-red-950/20 rounded transition"
                                            href="{{ route('magia.edit', ['id' => $magia['id'], 'tab' => 'magias']) }}">
                                            <svg class="w-5 h-5 fill-red-950" xmlns="http://www.w3.org/2000/svg"
                                                viewBox="0 0 640 640">
                                                <path
                                                    d="M535.6 85.7C513.7 63.8 478.3 63.8 456.4 85.7L432 110.1L529.9 208L554.3 183.6C576.2 161.7 576.2 126.3 554.3 104.4L535.6 85.7zM236.4 305.7C230.3 311.8 225.6 319.3 222.9 327.6L193.3 416.4C190.4 425 192.7 434.5 199.1 441C205.5 447.5 215 449.7 223.7 446.8L312.5 417.2C320.7 414.5 328.2 409.8 334.4 403.7L496 241.9L398.1 144L236.4 305.7zM160 128C107 128 64 171 64 224L64 480C64 533 107 576 160 576L416 576C469 576 512 533 512 480L512 384C512 366.3 497.7 352 480 352C462.3 352 448 366.3 448 384L448 480C448 497.7 433.7 512 416 512L160 512C142.3 512 128 497.7 128 480L128 224C128 206.3 142.3 192 160 192L256 192C273.7 192 288 177.7 288 160C288 142.3 273.7 128 256 128L160 128z" />
                                    ...
                                    <div class="p-2 text-base text-red-950 bg-transparent">
                                        <p class="italic mb-2 text-red-900/80 text-xs font-bold">Escola:
                                            {{ $magia['escola'] }} |
